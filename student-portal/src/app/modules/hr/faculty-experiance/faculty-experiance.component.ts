...
export class FacultyExperiance implements OnInit {
  ...
  saveFaculty() {
    ...
    if(this.id() ==0){
      ...
      this.hrService.createFacultyExperiance(data)
        .subscribe({
          next: () => {
      ...
      this.hrService.updateFacultyExperiance(this.id(),data)
        .subscribe({
          next: () => {
            this.isModalOpen.set(false);
            this.loadFacultyExperiance();
  ...
  viewFaculty(id: number) {
    ...
    this.hrService.getSingleFaculty(id)
      .subscribe({
        next: (res) => {
  ...
  deleteFaculty(id: number) {
    ...
    this.hrService.deleteFacultyExperiance(id)
      .subscribe({
        next: () => {
          this.loadFacultyExperiance();
        },
  ...
  loadFacultyExperiance() {
    ...
    this.hrService.getFacultyExperiance()
      .subscribe({
        next: (res: any) => {
  ...
  loadDepartments() {
    ...
    this.hrService.getDeptDropdown()
      .subscribe({
        next: (res: any) => {
  ...
  facultyDropdownData(){
    this.hrService.facultyDropdown()
      .subscribe({
        next: (res: any) => {
